        ...
        setTimeout(function(){
            ...
            for(var idx in Game.numbers){
                if(Game.numbers[idx].number == number){
                    ...
                    if(first_hit && animatingIdx == 0) Game.numbers[idx].multiplier(ctx);
                    if(last_hit && animatingIdx == KenoLogic.MaxDraw - 1) Game.numbers[idx].multiplier(ctx)
                }
            }
            animatingIdx++;
            if(animatingIdx == KenoLogic.MaxDraw){
                var winner = Game.payout.result(ctx, total_hit, first_hit, last_hit);
